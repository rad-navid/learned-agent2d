...
SamplePlayer::doShoot()
{
    ...
    const int VALID_PLAYER_THRESHOLD = 10;
    const ServerParam & SP = ServerParam::i();
    const PredictState state( wm );
    ...
    AbstractPlayerCont opponents
    	...
    	//LEFT ==1
		if(wm.ourSide()==1)
			ourScore=wm.gameMode().scoreLeft();
		else
			ourScore=wm.gameMode().scoreRight();
    	if ( ourScore > last_goal) {
	...
	*/
    /*
    bool cond1=FieldAnalyzer::can_shoot_from( holder->unum() == state.self().unum(),
            holder->pos(), state.getPlayerCont( new OpponentOrUnknownPlayerPredicate( state.ourSide() ) ),
            VALID_PLAYER_THRESHOLD ) ;
    bool cond2=SP.theirPenaltyArea().contains(wm.self().pos());

    if (cond1 && cond2 )
        {
    		bool result=forceShoot(this );
    		if(result)
    			return true;
        }
...
bool
SamplePlayer::forceShoot(rcsc::PlayerAgent * agent)
{
